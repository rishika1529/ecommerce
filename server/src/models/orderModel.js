...
export const createOrder = (address) => {
  ...
  const placeOrder = db.transaction(() => {
    const result = insertOrder.run(USER_ID, total, address.address_line, address.city, address.state, address.zip);
    ...
    for (const item of cartItems) {
      ...
      const updateResult = updateStock.run(item.quantity, item.product_id, item.quantity);
